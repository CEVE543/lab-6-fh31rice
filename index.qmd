...
fig, ax, plt = heatmap(ct1_slice;
    colormap = :seaborn_icefire_gradient,
    axis = (; aspect = DataAspect()),
    figure = (; size = (600, 300), fontsize = 24)
)
fig
```
```{julia}
...
lat_mask = (latitudes .>= lat_range[1]) .& (latitudes .<= lat_range[2])
lon_mask = (longitudes .>= lon_range[1]) .& (longitudes .<= lon_range[2])

lat_idx = findall(lat_mask)
lon_idx = findall(lon_mask)

tas_region = Array(tas[lat_idx, lon_idx, :])

```
```{julia}
weights = cosd.(latitudes[lat_idx])
weights ./= sum(weights)  


regional_mean = [mean(tas_region[:, :, t] .* weights', dims=(1,2)) for t in 1:size(tas_region, 3)]
regional_mean = [x[1] for x in regional_mean]
...
dates = collect(DateTime(2000, 1, 1):Hour(3):(DateTime(2000, 1, 1) + Hour(3)*(length(regional_mean)-1)))


fig = Figure(; size=(900, 400))
ax = Axis(fig[1, 1],
    title="Regional Mean Near-Surface Air Temperature (3-hourly)",
    xlabel="Year",
    ylabel="Temperature (K)")

lines!(ax, dates, regional_mean, color=:red, linewidth=2)
fig

```
```{julia}

tmax = argmax(regional_mean)
tmin = argmin(regional_mean)


println("Hottest 3-hour period: ", dates[tmax])
println("Coldest 3-hour period: ", dates[tmin])
```
```{julia}
hot_map  = tas[:, :, tmax]
cold_map = tas[:, :, tmin]
...
hot_map  = Array(tas[:, :, tmax])
cold_map = Array(tas[:, :, tmin])


lons = Array(group["lon"][:])
lats = Array(group["lat"][:])

fig = Figure(size = (1000, 400))

ax1 = Axis(fig[1, 1], title = "Hottest 3-hour Period (tas)")
ax2 = Axis(fig[1, 2], title = "Coldest 3-hour Period (tas)")

hm1 = heatmap!(ax1, lons, lats, hot_map; colormap = :seaborn_icefire_gradient)
hm2 = heatmap!(ax2, lons, lats, cold_map; colormap = :seaborn_icefire_gradient)

Colorbar(fig[1, 3], hm1; label = "Temperature (K)")
fig
